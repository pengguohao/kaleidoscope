 ...
 */
package com.pgh.kaleidoscope.core.secure.utils;
...
import com.pgh.kaleidoscope.core.secure.constant.SecureConstant;
import com.pgh.kaleidoscope.core.secure.exception.SecureException;
import com.pgh.kaleidoscope.core.secure.provider.IClientDetails;
...
public class SecureUtil {
	 ...
	 * @return BladeUser
	 */
	public static KaleidoscopeUser getUser() {
		HttpServletRequest request = WebUtil.getRequest();
		if (request == null) {
			return null;
		}
		// 优先从 request 中获取
		Object kaleidoscopeUser = request.getAttribute(BLADE_USER_REQUEST_ATTR);
		if (kaleidoscopeUser == null) {
			kaleidoscopeUser = getUser(request);
			if (kaleidoscopeUser != null) {
				// 设置到 request 中
				request.setAttribute(BLADE_USER_REQUEST_ATTR, kaleidoscopeUser);
			}
		}
		return (KaleidoscopeUser) kaleidoscopeUser;
	}
	 ...
	 * @return BladeUser
	 */
	public static KaleidoscopeUser getUser(HttpServletRequest request) {
		...
		KaleidoscopeUser kaleidoscopeUser = new KaleidoscopeUser();
		kaleidoscopeUser.setClientId(clientId);
		kaleidoscopeUser.setUserId(userId);
		kaleidoscopeUser.setTenantId(tenantId);
		kaleidoscopeUser.setAccount(account);
		kaleidoscopeUser.setRoleId(roleId);
		kaleidoscopeUser.setRoleName(roleName);
		kaleidoscopeUser.setUserName(userName);
		return kaleidoscopeUser;
	}
	...
	public static Long getUserId(HttpServletRequest request) {
		KaleidoscopeUser user = getUser(request);
		return (null == user) ? -1 : user.getUserId();
	}
